...
<div class="flex flex-col md:flex-row h-full flex-1 bg-[#f8fafc] text-slate-800 rounded-[2rem] overflow-hidden">
    {{-- Sidebar (Dark Theme) --}}
    <aside class="w-full md:w-64 flex-shrink-0 flex flex-col bg-white border-r border-slate-200">
        <div class="p-6 flex-1 flex flex-col">
            {{-- Brand --}}
            <div class="flex items-center gap-3 mb-8">
                <img src="{{ asset('logo.png') }}" alt="Logo" class="h-10 w-auto">
                <div>
                    <h2 class="text-sm font-bold text-slate-800 tracking-tight leading-tight">Admin Panel</h2>
            ...
            <nav class="space-y-2 flex-1">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl {{ request()->routeIs('admin.dashboard') ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100' }} text-xs font-bold transition-all">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                    Dashboard
                </a>
                <a href="{{ route('admin.orders') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl {{ request()->routeIs('admin.orders') ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100' }} text-xs font-medium transition-all">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Orders
                </a>
                <a href="{{ route('admin.users') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl {{ request()->routeIs('admin.users') ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100' }} text-xs font-medium transition-all">
            ...
            {{-- Bottom Actions --}}
            <div class="mt-8 space-y-2 border-t border-slate-200 pt-4">
    ...
    <main class="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        <div class="bg-white rounded-[1.5rem] p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] mb-8 flex justify-between items-center border border-slate-100">
            <div>
                <h1 class="text-2xl font-extrabold text-brand-700 tracking-tight mb-1">Overview</h1>
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3 pl-4 border-l border-slate-200">
                    ...
                    <div class="w-10 h-10 rounded-full bg-brand-100 border-2 border-white shadow-sm flex items-center justify-center text-brand-600 font-bold">
                        A
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <div class="bg-gradient-to-br from-indigo-50/50 to-white rounded-2xl p-6 shadow-sm border border-indigo-100/50 relative overflow-hidden">
                ...
                <h3 class="text-3xl font-extrabold text-brand-700">{{ number_format($stats['total']) }}</h3>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50/50 to-white rounded-2xl p-6 shadow-sm border border-purple-100/50 relative overflow-hidden">
                ...
                <h3 class="text-3xl font-extrabold text-brand-700">{{ number_format($stats['pending']) }}</h3>
            </div>
            ...
            <div class="bg-gradient-to-br from-sky-50/50 to-white rounded-2xl p-6 shadow-sm border border-sky-100/50 relative overflow-hidden">
                ...
                <h3 class="text-3xl font-extrabold text-brand-700">{{ number_format($stats['in_progress']) }}</h3>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50/50 to-white rounded-2xl p-6 shadow-sm border border-emerald-100/50 relative overflow-hidden">
                ...
                <h3 class="text-3xl font-extrabold text-brand-700">{{ number_format($stats['completed']) }}</h3>
            </div>
        </div>
                                    ...
                                    <td class="py-4 px-4 text-right">
                                        @if($req->status === 'pending')
                                            <form action="{{ route('admin.payouts.complete', $req) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="px-3 py-1.5 bg-brand-500 hover:bg-brand-600 text-white text-[10px] font-bold rounded-lg shadow-md transition-all">
                                                    Transfer Selesai ✓
                                                </button>
                                            </form>
                                        @else
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @foreach($orders->take(10) as $order)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    <td class="py-4 px-4 text-xs font-bold text-brand-700">#ORD-{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</td>
