...
<div class="flex h-full min-h-[calc(100vh-4rem)]">
    ...
    <main class="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        <div class="max-w-xl bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <h3 class="text-sm font-bold text-slate-800 mb-6">Detail Bank / E-Wallet</h3>
            <form action="{{ route('mitra.settings.update') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="bank_name" required value="{{ auth()->user()->bank_name }}" placeholder="Contoh: BCA / GoPay" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Nomor Rekening / No HP</label>
                    <input type="text" name="account_number" required value="{{ auth()->user()->account_number }}" placeholder="Masukkan nomor" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm">
                </div>
                
                <div class="pt-4 border-t border-slate-200">
                    <button type="submit" class="w-full py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold rounded-xl shadow-[0_4px_14px_rgba(64,106,175,0.25)] transition-all">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
